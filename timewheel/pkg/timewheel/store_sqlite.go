...
type GormTaskStore struct {
	db   *gorm.DB
	mu   sync.Mutex // 写入互斥锁，保证 SQLite 并发安全
}
...
func NewSQLiteStore(dbPath string) (*GormTaskStore, *GormHistoryStore, error) {
	// SQLite 连接配置：限制最大连接数为 1，避免并发写入冲突
	// 启用 WAL 模式提高并发读性能
...
// Flush 强制刷新所有待写入的记录（阻塞，用于测试）
func (s *GormHistoryStore) Flush() error {
	// 发送一个空记录来触发刷新（通过短暂等待让批量写入器处理）
