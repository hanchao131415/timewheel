...
func (tw *TimeWheel) Start() error {
	...
	// 初始化当前槽位索引为基于当前时间
	// 这样确保任务添加时的槽位计算与tick处理时的槽位检查保持同步
	initialSlot := int(time.Now().UnixMilli() / tw.interval.Milliseconds()) % tw.slotNum
	tw.currentSlot.Store(int64(initialSlot))
...
func (tw *TimeWheel) tick() {
	...
	// Use actual current time for accurate execution timing
	now := time.Now()
